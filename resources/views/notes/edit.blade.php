    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        <div class="text-center">
          <form class="row g-3" action="{{ route('notes.update',$note->id) }}" method="post">
            {!! csrf_field() !!}
            @method('put')
            <div class="col-md-12">
              <label for="inputEmail4" class="form-label">Titulo</label>
              <input type="text" name="titulo" class="form-control" value="{{$note->titulo}}" autofocus required>
            </div>
            <div class="col-md-12">
              <label for="inputEmail4" class="form-label">Contenido</label>
              <textarea type="text" name="contenido" class="form-control" value="{{$note->contenido}}"required></textarea>
            </div>
            <div class="col-md-6">
              <label for="inputPassword4" class="form-label">Fecha</label>
              <input type="date" name="fecha" class="form-control" value="{{$note->fecha}}" required>
            </div>
            <div class="col-6">
              <label for="inputCategories" class="form-label">Categoria</label>
              <select class="form-control" name='categoria_id' id="" value="{{$note->categoria_id}}" required>
